...
try:
    from hypomap.atlas.allen_api import ALLEN_SDK_AVAILABLE, get_atlas
    from hypomap.atlas.region_mapping import HYPOMAP_TO_CCF
except ImportError:
...
def assign_coordinates(
    ...
    if dataset == "mouse_hypomap":
    ...
    elif dataset in ("mouse_abc", "mouse_abc_extended", "mouse_abc_whole_brain"):
        ...
        if (
            ...
            raise ValueError(
                f"{dataset} dataset should already have x, y, z coordinates"
            )
    else:
...
def assign_human_coordinates(
    ...
    print(
        f"  Scaling UMAP coordinates to micrometers (scale: {scale_factor:.2f} µm/unit)"
    )
    print(
        f"  Target tissue size: {TISSUE_SIZE_UM:.0f} µm ({TISSUE_SIZE_UM/1000:.1f} mm)"
    )
    ...
    if "Sample_ID" in cells_df.columns:
        ...
        for donor in cells_df["_donor"].unique():
            ...
            if len(valid_sections) > 0:
                ...
                for idx in cells_df[donor_mask].index:
                    ...
                    cells_df.at[idx, "z"] = z_base + np.random.uniform(
                        0, SECTION_THICKNESS_UM
                    )
        ...
        print(
            f"  Cells with section info: {n_with_section}/{len(cells_df)} ({100*n_with_section/len(cells_df):.1f}%)"
        )
