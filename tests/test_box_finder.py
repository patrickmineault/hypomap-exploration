"""Tests for the local-diversity 3D box finder (src/diversity.py)."""
...
def _make_cells(x, y, z_slice, labels):
    ...
    return pd.DataFrame({
        'x': x, 'y': y, 'z_slice': z_slice, 'type': labels,
    })
...
class TestFindMaxMeanDiversityBox:
    ...
    def _run_search(self, df, metric='Shannon', volume_um3=500,
                    n_slices=1, symmetry='One-sided', radius=0.3,
                    midline_x=2.0):
        div_grids, count_grids, geo = build_local_diversity_grid(
        ...
        return find_max_mean_diversity_box(
            sum_sats, count_sats, geo, z_slices, n_slices,
            volume_um3, symmetry, midline_x=midline_x,
        )
    ...
    def test_finds_diverse_region_shannon(self):
        ...
        assert box_cx >= 2.0, f"Box center x={box_cx:.2f} should be >= 2.0 (diverse region)"
    ...
    def test_unit_conversion_sanity(self):
        ...
        assert box_area < 0.5, f"Box area {box_area:.2f} mm^2 should be small for 10 M um^3"
