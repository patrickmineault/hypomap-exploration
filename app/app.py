...
REGION_DESCRIPTIONS_PATH = DATA_DIR / "generated" / "mouse_common" / "region_descriptions.csv"
...
DATASET_PATHS = {
    'mouse_abc': {
        ...
        'lr_profile': DATA_DIR / "processed" / "mouse_abc" / "cluster_ligand_receptor_profile.parquet",
        'np_expr': DATA_DIR / "processed" / "mouse_abc" / "cluster_np_expression.parquet",
    },
    'mouse_abc_subcortical': {
        'cells': DATA_DIR / "processed" / "mouse_abc_subcortical" / "cells_with_coords.parquet",
        'regions': DATA_DIR / "processed" / "mouse_abc_subcortical" / "coronal_atlas_regions.json",
        'lr_profile': DATA_DIR / "processed" / "mouse_abc_subcortical" / "cluster_ligand_receptor_profile.parquet",
        'np_expr': DATA_DIR / "processed" / "mouse_abc_subcortical" / "cluster_np_expression.parquet",
    },
}
...
def load_ccf_region_colors():
    ...
    cache_base = project_data / "raw" / "abc_atlas_cache" / "metadata" / "Allen-CCF-2020" / "20230630" / "views"
    color_path = cache_base / "parcellation_to_parcellation_term_membership_color.csv"
    acronym_path = cache_base / "parcellation_to_parcellation_term_membership_acronym.csv"

    if not color_path.exists() or not acronym_path.exists():
        print(f"Warning: CCF color tables not found at {cache_base}, using empty color map")
        return {}
    ...
    mapping = merged.drop_duplicates(subset='structure').set_index('structure')['structure_color'].to_dict()
...
def load_dataset_bundle(dataset_name):
    ...
    if missing_regions:
        ...
        for i, region in enumerate(sorted(missing_regions)):
            region_colors[region] = extended_palette[(existing_count + i) % len(extended_palette)]
        print(f"[{dataset_name}] Auto-assigned colors for {len(missing_regions)} new regions")
...
def create_app():
    ...
    hormone_systems, hormone_names = load_hormone_systems() if ENABLE_HORMONE_MODE else ({}, [])
    ...
    for ds_name in DATASET_PATHS:
        try:
            ...
            print(f"  Loaded {ds_name}: {len(datasets[ds_name]['cells_df']):,} cells, {len(datasets[ds_name]['slices'])} slices")
    ...
    default_dataset = 'mouse_abc_subcortical' if 'mouse_abc_subcortical' in datasets else list(datasets.keys())[0]
    ...
    for ds_name, ds_data in datasets.items():
        print(f"  [{ds_name}] Cells: {len(ds_data['cells_df']):,}, Slices: {len(ds_data['slices'])}")
...
_running_as_main = (
    _main_mod is not None and
    getattr(_main_mod, '__file__', None) is not None and
    _main_mod.__file__.endswith('app.py')
)
