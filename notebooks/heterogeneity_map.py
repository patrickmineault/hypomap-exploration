...
def _(Path, json, np, pd):
    ...
    cells_df = cells_df[~cells_df['region'].str.contains('unassigned', case=False)].copy()
    ...
    print(f"Granularity levels: class ({cells_df['class'].nunique()}), "
    ...
    print(f"Neuropeptide SD filter: kept {len(_kept_genes)}/{len(_np_genes)} genes "
          f"(SD > {_sd_thresh:.3f})")
...
def _(
    ...
    print(f"Computed {_metric} ({_granularity}) for {len(slice_results)} slices, "
...
def _(mo):
    radius_slider = mo.ui.slider(
        start=0.05, stop=0.5, step=0.05, value=0.15,
        label="Integration radius (mm)"
    )
    ...
    mode_dropdown = mo.ui.dropdown(
        options=["Grid", "Per-cell"],
        value="Grid",
        label="Computation mode"
    )
    grid_res_slider = mo.ui.slider(
        start=20, stop=200, step=10, value=100,
...
def _(
    ...
    for _i, _z in enumerate(_sorted_slices):
        ...
        if _use_images and _z in slice_images:
            ...
            _ax.imshow(
                _img, cmap=_cmap, norm=_norm,
                extent=[_xmin_s - _half_dx, _xmax_s + _half_dx,
                        _ymax_s + _half_dy, _ymin_s - _half_dy],
            ...
            _sx, _sy, _sv = slice_results[_z]
            _ax.scatter(
                _sx, _sy, c=_sv, s=4, cmap='viridis',
        ...
        if _z in centroids:
            for _region, (_cx, _cy) in centroids[_z].items():
                _ax.text(
                    _cx, _cy, _region, fontsize=5, color='white',
                    ha='center', va='center',
    ...
    _fig.tight_layout(rect=[0, 0, 0.92, 0.96])
...
def _(cells_df, compute_diversity, granularity_dropdown, np, pd):
    ...
    # Level 2: region
    for (_div, _reg), _grp in _hier_df.groupby(
        ['parcellation_division', 'region']
    ):
        _m = _diversity_for_group(_grp[_granularity])
        _rows.append({
            'level': 'region',
    ...
    diversity_table = diversity_table.sort_values(
        ['division', '_level_sort', 'region', 'substructure']
    ).drop(columns='_level_sort').reset_index(drop=True)

    print(f"Diversity table: {len(diversity_table)} rows "
...
def _(diversity_table, mo):
    mo.ui.table(
        diversity_table[
            ['level', 'division', 'name', 'n_cells',
             'Shannon', 'Simpson', 'Type count']
        ],
...
def _(diversity_table, go):
    ...
    _traces = [{
        ...
        'text': [f"{x} > {y}" for x,y in zip(_df['division'].tolist(), _df['name'].tolist())],
...
def _(go, make_subplots, math):
    def make_bbox_figure(z_slices, x_range, y_range, boundaries, centroids, title):
        ...
        for i, z in enumerate(z_slices):
            ...
            all_traces.append({
                'type': 'scattergl',
                'x': [x0 - pad, x1 + pad], 'y': [y0 - pad, y1 + pad],
                'mode': 'markers', 'marker': {'size': 0.1, 'opacity': 0},
            ...
            if zr in boundaries:
                for rgn, coords in boundaries[zr].items():
                    ...
                    all_shapes.append({
                        'type': 'path', 'path': path, 'xref': xa, 'yref': ya,
                        'line': {'color': 'rgba(100,100,100,0.6)', 'width': 1.5},
                        'fillcolor': 'rgba(0,0,0,0)',
                    })
            if zr in centroids:
                for rgn, (cx, cy) in centroids[zr].items():
                    all_annotations.append({
                        'x': cx, 'y': cy, 'text': rgn, 'showarrow': False,
                        'font': {'size': 9, 'color': 'white'},
...
def _():
    from src.diversity import (
        build_local_diversity_grid, build_diversity_sats,
        find_max_mean_diversity_box, mask_excluded_regions,
    )
    return
