...
def _():
    ...
    return cKDTree, go, make_subplots, math, mo, np, pd


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    # Whole-Brain Heterogeneity Map (Langlieb et al. 2023, Slide-seq)

    Interactive visualization of **cell-type diversity** across the entire mouse brain
    using ~1.7M RCTD-mapped Slide-seq beads from Langlieb et al. 2023 (101 coronal pucks
    with Allen CCF coordinates).

    For each coronal slice, diversity is computed within an adjustable integration radius
    at a chosen taxonomic granularity (neurotransmitter / cluster / CCF_acronym).
...
def _(mo, np, pd):
    # Load cell data
    _data_dir = mo.notebook_dir() / ".." / "data" / "processed" / "mouse_langlieb"
    cells_df = pd.read_parquet(_data_dir / "cells_with_coords.parquet")

    # Round z for discrete slices (should already be rounded, but ensure consistency)
    cells_df["z_slice"] = cells_df["z"].round(2)

    # Load region boundaries if available (optional for Langlieb)
    _regions_path = _data_dir / "coronal_atlas_regions.msgpack"
    boundaries = {}
    centroids = {}
    if _regions_path.exists():
        import msgpack

        with open(_regions_path, "rb") as _f:
            _region_data = msgpack.unpackb(_f.read(), raw=False)
        # Keys must be float32 to match z_slice values from the parquet
        boundaries = {
            np.float32(k): {
    ...
    print(f"Loaded {len(cells_df):,} beads across {len(all_slices)} slices")
    print(
        f"Granularity levels: neurotransmitter ({cells_df['neurotransmitter'].nunique()}), "
        f"cluster ({cells_df['cluster'].nunique()}), "
        f"CCF_acronym ({cells_df['CCF_acronym'].nunique()})"
    )
    print(
        f"Main regions: {sorted(cells_df['region'].unique())}"
    )
    return all_slices, boundaries, cells_df, centroids


@app.cell
def _(all_slices, cKDTree, cells_df, go, mo, np):
    # Per-slice diagnostics: bead count and neighborhood density
    _rows = []
    ...
    for _z in all_slices:
        ...
        _xy = _raw_xy[_mask]
        _n = int(_mask.sum())
        _mean_nbrs = float("nan")
        if _n >= 10:
            _tree = cKDTree(_xy)
            # Sample up to 200 beads for speed
            _rng = np.random.default_rng(42)
            _sample = _rng.choice(len(_xy), min(200, len(_xy)), replace=False)
            _nbrs = _tree.query_ball_point(_xy[_sample], 0.15)
            _mean_nbrs = np.mean([len(n) for n in _nbrs])
        _rows.append({"z": round(float(_z), 2), "n_beads": _n, "mean_neighbors_r015": round(_mean_nbrs, 1)})
    ...
    _diag = go.Figure(
        {
            "data": [
                {
                    "type": "bar",
                    "x": [r["z"] for r in _rows],
                    "y": [r["n_beads"] for r in _rows],
                    "name": "Beads per slice",
                    "marker": {"color": "#4682B4"},
                    "yaxis": "y",
                },
                {
                    "type": "scatter",
                    "x": [r["z"] for r in _rows],
                    "y": [r["mean_neighbors_r015"] for r in _rows],
                    "name": "Mean neighbors (r=0.15mm)",
                    "mode": "lines+markers",
                    "marker": {"color": "#e41a1c", "size": 4},
                    "line": {"color": "#e41a1c"},
                    "yaxis": "y2",
                },
            ],
            "layout": {
                "title": {"text": "Slice diagnostics: bead density and neighborhood size"},
                "xaxis": {"title": "AP position (mm)"},
                "yaxis": {"title": "Beads per slice", "side": "left"},
                "yaxis2": {
                    "title": "Mean neighbors (r=0.15mm)",
                    "side": "right",
                    "overlaying": "y",
                },
    ...
    Bead density varies **{min(r['n_beads'] for r in _rows):,}–{max(r['n_beads'] for r in _rows):,}** beads per puck.
    Low-density pucks have fewer neighbors per grid point, which biases Shannon entropy downward
    (fewer samples → fewer observed types). Large slice-to-slice jumps in the diversity maps
    usually correspond to adjacent pucks with very different bead counts.
    """
    )
    _diag
    return
...
def _(cKDTree, np):
    ...
    def compute_heterogeneity_grid(
        xy, int_labels, radius, metric, grid_res, max_neighbors=None, rng=None
    ):
        ...
        Args:
            ...
            max_neighbors: if set, cap neighbors per grid point to this count
            rng: numpy random generator (required if max_neighbors is set)
        ...
        for _i, _nbrs in enumerate(_neighbors):
            if len(_nbrs) >= 2:
                if max_neighbors and len(_nbrs) > max_neighbors:
                    _nbrs = list(rng.choice(_nbrs, max_neighbors, replace=False))
                _values[_i] = compute_diversity(int_labels[_nbrs], metric)

        return _grid_points[:, 0], _grid_points[:, 1], _values

    def compute_heterogeneity_percell(
        xy, int_labels, radius, metric, max_neighbors=None, rng=None
    ):
        ...
        Args:
            ...
            radius: integration radius in mm
            metric: diversity metric name
            max_neighbors: if set, cap neighbors per cell to this count
            rng: numpy random generator (required if max_neighbors is set)
...
def _(
    ...
    _density_match = density_match_checkbox.value
    ...
    # --- First pass: estimate per-slice mean neighbor count for density matching ---
    _slice_mean_nbrs = {}
    if _density_match:
        for _z in all_slices:
            _mask = _raw_z == _z
            ...
            if len(_xy) < 20:
                continue
            _tree_tmp = cKDTree(_xy)
            _sample = _rng.choice(len(_xy), min(100, len(_xy)), replace=False)
            _nbrs_tmp = _tree_tmp.query_ball_point(_xy[_sample], _radius)
            _slice_mean_nbrs[_z] = np.mean([len(n) for n in _nbrs_tmp])

        if _slice_mean_nbrs:
            _target_nbrs = 2.0 * min(_slice_mean_nbrs.values())
        else:
            _target_nbrs = float("inf")

    # Per-neighborhood cap: when density matching, limit neighbors per query point
    _max_nbrs = None
    if _density_match and _slice_mean_nbrs:
        _max_nbrs = int(_target_nbrs)
    ...
    for _z in all_slices:
        ...
        if _mode == "Grid":
            _sx, _sy, _sv = compute_heterogeneity_grid(
                ...
                max_neighbors=_max_nbrs, rng=_rng,
            )
    ...
    _dm_msg = ""
    if _density_match and _slice_mean_nbrs:
        _dm_msg = f", density-matched (target={_target_nbrs:.0f} neighbors)"
    print(
        f"Computed {_metric} ({_granularity}) for {len(slice_results)} slices, "
        f"mode={_mode}, radius={_radius}mm{_dm_msg}"
    )
...
def _(mo):
    ...
    granularity_dropdown = mo.ui.dropdown(
        options=["neurotransmitter", "cluster", "CCF_acronym"],
        value="cluster",
    ...
    density_match_checkbox = mo.ui.checkbox(
        value=True, label="Density-match slices"
    )
    overview_step_slider = mo.ui.slider(
        start=1, stop=20, step=1, value=4, label="Overview: show every Nth slice"
    )
    ...
    {mo.hstack([density_match_checkbox, overview_step_slider], justify="start")}
    ...
    **Density-match**: subsample dense slices so neighborhood counts are roughly equal across pucks, reducing slice-to-slice discontinuities.
    """
    )
    return (
        density_match_checkbox,
...
def _(
    ...
    # Multi-slice overview figure — show every Nth slice
    _all_sorted = sorted(slice_results.keys())
    _sorted_slices = _all_sorted[::overview_step_slider.value]

    _n_slices = len(_sorted_slices)
    _ncols = 4
    _nrows = math.ceil(_n_slices / _ncols)

    _max_vspacing = 1.0 / max(_nrows - 1, 1)
    _fig = make_subplots(
        rows=_nrows,
        cols=_ncols,
        subplot_titles=[f"y={z:.2f}" for z in _sorted_slices],
        horizontal_spacing=0.03,
        vertical_spacing=min(0.06, _max_vspacing * 0.9),
    )
...
def _(
    ...
    if _z is not None and _z in slice_results:
        ...
        # Global axis ranges across all slices so the view stays fixed when scrubbing
        _all_x = np.concatenate([slice_results[k][0] for k in slice_results])
        _all_y = np.concatenate([slice_results[k][1] for k in slice_results])
        _pad = 0.2
        _x_range = [float(_all_x.min()) - _pad, float(_all_x.max()) + _pad]
        _y_range = [float(_all_y.max()) + _pad, float(_all_y.min()) - _pad]  # reversed
...
def _(
    ...
    _metric_name = metric_dropdown.value
    _gran_name = granularity_dropdown.value
    _sorted_slices = sorted(slice_results.keys())[::overview_step_slider.value]
...
def _(mo):
    ...
    at every level of the Langlieb hierarchy:

    - **CCF_acronym**: finest parcellation (e.g. ARH, VMH, DCO)
    - **region**: broadest level (CB, HY, TH, CTX, MB, MY, etc.)
...
def _(cells_df, compute_diversity, granularity_dropdown, np, pd):
    ...
    _hier_df = cells_df[
        [_granularity, "region", "CCF_acronym"]
    ].copy()
    ...
    # Level 1: CCF_acronym (finest)
    for (_mr, _acr), _grp in _hier_df.groupby(
        ["region", "CCF_acronym"]
    ):
    ...
    # Level 2: region (broadest)
    for _mr, _grp in _hier_df.groupby("region"):
        _m = _diversity_for_group(_grp[_granularity])
        _rows.append(
            {
                "level": "region",
    ...
    diversity_table["Type count"] = diversity_table["Type count"].fillna(0).astype(int)

    # Sort: region rows first (by name), then CCF_acronym
    _level_order = {"region": 0, "CCF_acronym": 1}
    diversity_table["_level_sort"] = diversity_table["level"].map(_level_order)
    diversity_table = (
        diversity_table.sort_values(
            ["region", "_level_sort", "CCF_acronym"]
        )
...
def _(diversity_table, mo):
    mo.ui.table(
        diversity_table[
            ["level", "region", "name", "n_cells", "Shannon", "Simpson", "Type count"]
        ],
...
def _(diversity_table, go):
    _df = diversity_table
    _colors = {"region": "#e41a1c", "CCF_acronym": "#4daf4a"}
    ...
    _traces = [
        {
            ...
            "text": [
                f"{x} > {y}"
                for x, y in zip(_df["region"].tolist(), _df["name"].tolist())
            ],
...
def _(mo):
    ...
    **Effect of granularity**:
    - **Neurotransmitter**: Broadest grouping (Ex, Inh, Chol, Dop, Nor, Ser, Non-neuronal). Low diversity = dominated by one NT class
    - **Cluster**: Finest level (~1,937 RCTD cell types). Almost everywhere shows high diversity
    - **CCF_acronym**: Diversity of anatomical region labels within the radius — reveals how many distinct brain structures intermingle spatially
    ...
    **Tips**:
    - Start with neurotransmitter granularity and 0.15mm radius for a good initial view
    ...
    - The whole-brain coverage reveals diversity gradients across all 12 main brain regions
    """)
    return
